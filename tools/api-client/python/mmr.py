...
button_rating_csv = "buttons-12-21.csv"
player_rating_csv = "players-12-21.csv"
update_forum=True

# _start_date = date(2001,1,1)
_start_date = date(2022,1,1)
#_stop_date = date(2022, 1, 15) # 10)
# _stop_date = date(2022, 12, 23) # 20)
_stop_date = date(2022, 2, 1) - timedelta(days=1)
_forum="1021,23580"
_forum_thread=1021
_forum_post=23580
_forum_text=""
...
_strategies = {"all": Strategy(
  ...
  "hitlistjun21": Strategy(
      lambda game: (buttons[game['buttonNameA']][
                      'buttonName'] in hitlistjun21_studiofoglio and
                    buttons[game['buttonNameB']][
                      'buttonName'] in hitlistjun21_wonderland) or
                   (buttons[game['buttonNameA']][
                      'buttonName'] in hitlistjun21_wonderland and
                    buttons[game['buttonNameB']][
                      'buttonName'] in hitlistjun21_studiofoglio)),

  "hitlistjul21": Strategy(
      lambda game: buttons[game['buttonNameA']][
                     'buttonName'] in hitlistjul21_buttons and
                   buttons[game['buttonNameB']][
                     'buttonName'] in hitlistjul21_buttons),

  "hitlistaug21": Strategy(
      lambda game: (buttons[game['buttonNameA']][
                     'buttonSet'] == "Blademasters" and
                   buttons[game['buttonNameB']][
                     'buttonSet'] == "Uptown") or
                  (buttons[game['buttonNameA']][
                      'buttonSet'] == "Uptown" and
                  buttons[game['buttonNameB']][
                    'buttonSet'] == "Blademasters")),

  "hitlistsep21": Strategy(
      lambda game: buttons[game['buttonNameA']][
                     'buttonName'] in hitlistsep21_buttons and
                   buttons[game['buttonNameB']][
                     'buttonName'] in hitlistsep21_buttons),


  "hitlistoct21": Strategy(
      lambda game: (buttons[game['buttonNameA']][
                      'buttonSet'] == "Vampyres" and
                    buttons[game['buttonNameB']][
                      'buttonSet'] == "Cowboy Bebop") or
                   (buttons[game['buttonNameA']][
                      'buttonSet'] == "Cowboy Bebop" and
                    buttons[game['buttonNameB']][
                      'buttonSet'] == "Vampyres")),


  "hitlistnov21": Strategy(
    lambda game: buttons[game['buttonNameA']][
                   'buttonName'] in hitlistnov21_buttons and
                 buttons[game['buttonNameB']][
                   'buttonName'] in hitlistnov21_buttons),

  "hitlistdec21": Strategy(
    lambda game: (buttons[game['buttonNameA']][
                    'buttonSet'] == "Big Top" and
                  buttons[game['buttonNameB']][
                    'buttonSet'] == "Renaissance") or
                 (buttons[game['buttonNameA']][
                    'buttonSet'] == "Renaissance" and
                  buttons[game['buttonNameB']][
                    'buttonSet'] == "Big Top")),

  "hitlistjan22": Strategy(
    lambda game: buttons[game['buttonNameA']][
                    'buttonSet'] == "CyberSuit Corp" or
                 buttons[game['buttonNameB']][
...
hitlistjun21_studiofoglio = ["Agatha", "Bang", "Brigid", "Buck Godot", "Dixie", "Gil", "Growf", "Jorgi", "Klaus", "Krosp", "O-Lass", "Phil", "The James Beast", "Von Pinn"]
hitlistjun21_wonderland = ["Alice", "Mad Hatter", "Queen Of Hearts", "The Jabberwock", "Tweedledum+dee", "White Rabbit"]

hitlistjul21_buttons = ["ABCGi", "CestWhat", "Heath", "Santiago", "fxdirect", "Notorious", "Zomulgustar", "Torch", "hansolav", "NoopMan", "barswanian", "Grivan", "GorgorBey", "TheMachine", "Darthcliff", "dexx", "wtrollkin"]
hitlistsep21_buttons = ["Snuff", "mgatten", "Discordia", "skapheles", "gslamm", "nelde", "spindisc", "relsqui", "icarus", "bobby 5150", "kestrel", "Cristofore", "trifecta", "IIconfused", "Weylan", "Zotmeister", "fendrin", "Zegota", "Squiddhartha", "roujin27"]
hitlistnov21_buttons = ["Wastenott", "Max", "Rumbles", "Super Germ", "Synthia", "eon", "Invisible Man", "Mushu", "Golo", "Skomp", "Peace", "Montgomery", "Wyoming", "Trogdor", "Mister Peach", "Binder", "Limax", "stoooooooo", "gman97216", "Cthulhu"]
...
def do_the_ratings(game):
  ...
  (wprate, wbrate), (lprate, lbrate) = rate(
      [[wprate, wbrate], [lprate, lbrate]])
  ...
  if highlight_player in [wplay, lplay] or highlight_general:
    ...
    if highlight_player in [wplay, lplay] or worth_highlighting:
      if highlight_fmt == 'forum':
        ...
        print_and_collect(f"[game={game['gameId']}] {wprint} vs {lprint} @{winprob*100:0.0f}%")
...
def print_report(ratings, total_game_count, total_win_count, itemized_counts,
  ...
  print_and_collect(f"[quote][b]{tag_primary.upper()} LEADERBOARD[/b]\n[i]rank [rating] medal {tag_primary}(gamecount, winrate?)[/i]\n")

  min_games_played = 5
  leaderboard = dict(
      filter(lambda elem: total_game_count.get(elem[0], 0) >= min_games_played,
             ratings.items()))
  ...
  print_and_collect(f"[quote][b]{tag_primary.upper()} PLACEMENT[/b] Not yet played {min_games_played} games\n[i]{tag_primary}(games)[/i]\n")

  leaderboard_placement = dict(
      filter(lambda elem: total_game_count.get(elem[0], 0) < min_games_played and total_game_count.get(elem[0], 0) >= 1,
             ratings.items()))
  leaderboard_placement = {k: v for k, v in
                           sorted(leaderboard_placement.items(),
                                  key=lambda item: total_game_count.get(
                                      item[0], 0),
                                  reverse=True)}
...
def json_dump(name, total_game_count, total_win_count, itemized_counts):
  with open(name, 'w', encoding='utf-8') as f:
    json.dump({'total_game_count': total_game_count,
               'total_win_count': total_win_count,
               'itemized_counts': itemized_counts}, f, ensure_ascii=False,
              indent=2)
...
if __name__ == "__main__":
  ...
  json_dump('button_data.json', buttons_total_game_count,
            buttons_total_win_count, buttons_player_counts)
  json_dump('player_data.json', buttons_total_game_count,
            buttons_total_win_count, buttons_player_counts)

  print_report(player_ratings, players_total_game_count,
               players_total_win_count, players_button_counts, 'player',
               'button')
  print_and_collect('\n')
  print_report(button_ratings, buttons_total_game_count,
               buttons_total_win_count, buttons_player_counts, 'button',
               'player')
  ...
  if update_forum:
    ...
    if current_post['body'].split("\n")[0]==_forum_text.split("\n")[0] :
      print(f"no need to update https://www.buttonweavers.com/ui/forum.html#!threadId={_forum_thread}&postId={_forum_post}")
    else:
      bm.wrap_edit_forum_post(_forum_post, _forum_text)
      print(f"updated https://www.buttonweavers.com/ui/forum.html#!threadId={_forum_thread}&postId={_forum_post}")
