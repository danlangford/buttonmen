...
""" % (
      ...
      entry['button1'].replace("'", "\\'"),
      entry['button2'].replace("'", "\\'"),
      entry['max_wins']))
...
""" % (
      ...
      entry['roundNumber'], entry['attackType'], entry['attacker'],
      entry['defender'],
      php_turbo_vals))
  ...
  def _write_php_diff_opt_request_array(self, suffix, newval, oldval):
    ...
    if newval != oldval:
      if type(newval) == dict:
        self.f.write("        $expData%s = array(%s);\n" % (
          suffix, ', '.join(['"%s" => array(%s)' % (
            k, ', '.join([str(subv) for subv in v])) for [k, v] in
                             sorted(newval.items())])))
      else:
        self.f.write("        $expData%s = array(%s);\n" % (
          suffix, ', '.join(
    ...
    for dnum in range(len(newdice)):
      ...
      for dkey in sorted(newdie.keys()):
        ...
        if dkey in STRING_KEYS:
          self._write_php_diff_string_key(suffix, newdie[dkey],
                                          olddie[dkey])
        elif dkey in NUMERIC_KEYS:
          self._write_php_diff_numeric_key(suffix, newdie[dkey],
                                           olddie[dkey])
    ...
    for pkey in sorted(newdata.keys()):
      ...
      if pkey in ['activeDieArray', 'capturedDieArray',
        ...
        self._write_php_diff_player_data_die_array(pnum, pkey,
                                                   newdata[pkey],
                                                   oldval)
      elif pkey in ['gameScoreArray', 'swingRequestArray',
                    'prevSwingValueArray', ]:
        self._write_php_diff_flat_dict_key(suffix, newdata[pkey],
                                           olddata[pkey])
      elif pkey in ['button', ]:
        self._write_php_diff_flat_dict_key(suffix, newdata[pkey],
                                           olddata[pkey], True)
      elif pkey in ['prevOptValueArray', ]:
        self._write_php_diff_prev_opt_value_array(suffix, newdata[pkey],
                                                  olddata[pkey])
      elif pkey in ['optRequestArray', 'turboSizeArray', ]:
        self._write_php_diff_opt_request_array(suffix, newdata[pkey],
                                               olddata[pkey])
      elif pkey in NUMERIC_KEYS:
        self._write_php_diff_numeric_key(suffix, newdata[pkey],
                                         olddata[pkey])
  ...
  def _write_php_json_diff(self, newobj, oldobj):
    for key in sorted(newobj.keys()):
      ...
      if key in NUMERIC_KEYS:
        self._write_php_diff_numeric_key(suffix, newobj[key],
                                         oldobj[key])
      elif key in STRING_KEYS:
        self._write_php_diff_string_key(suffix, newobj[key],
                                        oldobj[key])
        ...
        self._write_php_diff_flat_array_key(suffix, newobj[key],
                                            oldobj[key])
      elif key == 'gameSkillsInfo':
        self._write_php_diff_game_skills_info(suffix, newobj[key],
                                              oldobj[key])
      else:
...
"""
    ...
    for i in range(len(attacker_combos)):
      for j in range(len(defender_combos)):
        ...
        for k in range(len(attacker_dice)):
          if k in attacker_combos[i]:
            test_attackers.append(attacker_dice[k])
          else:
            test_non_attackers.append(attacker_dice[k])
        test_defenders = [defender_dice[k] for k in defender_combos[j]]
        if validate_fn(test_attackers, test_defenders,
                       test_non_attackers):
  ...
  def _can_gain_initiative_using_focus_dice(self, player_dice, opponent_dice):
    skip_initiative_skills = ['Rage', 'Slow', 'Stinger', 'Trip',
                              'Warrior', ]
    ...
    for die in player_dice:
      ...
      if 'Focus' in die['skills']:
        if ',' in die['recipe']:
          ...
          player_initiative_values.append(1)
      else:
  ...
  def _is_valid_attack_of_type_Berserk(self, attackers, defenders,
    ...
    attack_skills = {
      ...
      'ok': [
        ...
        'Jolt', 'Konstant', 'Mad', 'Maximum', 'Mighty', 'Mood',
        'Morphing',
        'Null', 'Ornery', 'Poison', 'Queer', 'Radioactive', 'Rage',
        'Rush',
        'Shadow', 'Slow', 'Speed', 'Stinger', 'TimeAndSpace', 'Turbo',
        'Trip',
        'Value', 'Weak',
      ],
      'no': ['Stealth', 'Warrior', ],
    ...
    defend_skills = {
      'ok': [
        ...
        'Insult', 'Jolt', 'Konstant', 'Mad', 'Maximum', 'Mighty',
        'Mood',
  ...
  def _is_valid_attack_of_type_Power(self, attackers, defenders,
    ...
    attack_skills = {
      'ok': [
        ...
        'Insult', 'Jolt', 'Mad', 'Maximum', 'Mighty', 'Mood',
        'Morphing',
        'Null', 'Ornery', 'Poison', 'Queer', 'Radioactive', 'Rage',
        'Rush',
        'Slow', 'Speed', 'Stinger', 'TimeAndSpace', 'Turbo', 'Trip',
        'Value',
        'Weak',
      ],
  ...
  def _is_valid_attack_of_type_Shadow(self, attackers, defenders,
    ...
    attack_skills = {
      ...
      'ok': [
        ...
        'Mood',
        'Morphing', 'Null', 'Ornery', 'Poison', 'Queer', 'Radioactive',
        'Rage', 'Rush', 'Slow', 'Speed', 'Stinger', 'TimeAndSpace',
        'Trip',
        'Turbo', 'Value', 'Weak',
      ],
      'no': ['Stealth', 'Warrior', ],
  ...
  def _generate_turbo_val_arrays(self, part_attackers, attack_type):
    ...
    if type(attackerData['turboSizeArray']) == list:
      ...
      raise ValueError("Unexpected type: %s in data: %s" % (
        attackerData['turboSizeArray'], attackerData))
  ...
  def _is_valid_attack_of_type_Skill(self, attackers, defenders,
    ...
    attack_skills = {
      'ok': [
        ...
        'Null', 'Ornery', 'Poison', 'Queer', 'Radioactive', 'Rage',
        'Rush', 'Shadow', 'Slow', 'Speed', 'Stealth', 'Stinger',
        'TimeAndSpace',
    ...
    defend_skills = {
      'ok': [
        ...
        'Konstant', 'Mad', 'Maximum', 'Mighty', 'Mood', 'Morphing',
        'Null',
        'Ornery', 'Poison', 'Queer', 'Radioactive', 'Rage', 'Rush',
        'Shadow',
    ...
    for non_attacker in non_attackers:
      if 'Fire' in non_attacker['skills']:
        ...
        if ',' in non_attacker['recipe']:
          range_max = non_attacker['value'] - 1
        else:
          range_max = non_attacker['value']
  ...
  def _is_valid_attack_of_type_Speed(self, attackers, defenders,
    ...
    attack_skills = {
      ...
      'ok': [
        ...
        'Mood',
        'Morphing', 'Null', 'Ornery', 'Poison', 'Queer', 'Radioactive',
        'Rage', 'Rush', 'Shadow', 'Slow', 'Stinger', 'TimeAndSpace',
        'Trip',
        'Turbo', 'Value', 'Weak',
      ],
      'no': ['Stealth', 'Warrior', ],
  ...
  def _generate_attack_array(self, part_attackers, part_defenders):
    ...
    attack = {}
    for i in range(len(self.loaded_data['playerDataArray'][self.attacker][
                         'activeDieArray'])):
      if i in part_attackers:
        value = 'true'
      else:
    ...
    for i in range(len(self.loaded_data['playerDataArray'][self.defender][
                         'activeDieArray'])):
      if i in part_defenders:
        value = 'true'
      else:
  ...
  def _game_action_adjust_fire_dice_player(self, b, playerData, opponentData):
    ...
    if choice == 'turndown':
      ...
      while still_needed > 0:
        ...
        turndown_to.setdefault(die_idx,
  ...
  def _game_action_specify_dice_player(self, b, playerData):
    ...
    if playerData['swingRequestArray']:
      unique_swing_among_swing = playerData['button']['name'] in [
        'Guillermo', 'Oregon']
      unique_swing_among_dice = playerData['button']['name'] == 'Gordo'
  ...
  def _game_action_start_turn_player(self, b, attackerData, defenderData):
    ...
    turbo_vals = self._generate_turbo_val_arrays(attacker_indices,
                                                 chosenAttackType)
    b.login()
    ...
    self.log.append({
      ...
      'all_attackers': self.loaded_data['playerDataArray'][self.attacker][
        'activeDieArray'],
      'all_defenders': self.loaded_data['playerDataArray'][self.defender][
        'activeDieArray'],
  ...
  def next_game_action(self):
    ...
    while state not in ['END_GAME', 'CANCELLED', ]:
      if state == 'START_TURN':
        self._game_action_start_turn()
      elif state == 'SPECIFY_DICE':
        self._game_action_specify_dice()
      elif state == 'REACT_TO_INITIATIVE':
        self._game_action_react_to_initiative()
      elif state == 'CHOOSE_RESERVE_DICE':
        self._game_action_choose_reserve_dice()
      elif state == 'CHOOSE_AUXILIARY_DICE':
        self._game_action_choose_auxiliary_dice()
      elif state == 'ADJUST_FIRE_DICE':
