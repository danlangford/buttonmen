...
tournament = challonge.tournaments.show(
    "thallium-event3b", include_participants=1, include_matches=1)
tid = tournament["id"]
print(
...
def update_scores():
  ...
  while keep_going:
    ...
    search = bm.wrap_search_game_history(
        ...
        lastMoveMin=int(
            datetime(2021, 1, 19, 0, 0, 0, tzinfo=timezone.utc).timestamp()),
    )
...
def create_games():
  ...
  for match in allMatches:
    if match["state"] == "open" and match["player1_id"] and match["player2_id"]:
      ...
      game = bm.wrap_create_game(
          p1,
          p2,
          "buttonbot",
          "buttonbot2",
          f"TLMadnessSE-B {mid} https://thallium.challonge.com/event3b",
      )
      gid = game["gameId"]
      print(f"{mid},{p1},{p2},{gid}")
      challonge.attachments.create(
          tid, mid, url=f"http://buttonweavers.com/ui/game.html?game={gid}")
